...
public class MailService {
	...
	@Async
	public void sendEmailWithAttachement(String to, String subject, String content, boolean isMultipart, boolean isHtml,
			byte[] file) {
		log.debug("Send email[multipart '{}' and html '{}'] to '{}' with subject '{}' and content={}", isMultipart,
				isHtml, to, subject, content);
	...
	public void sendInvoiceMail(Invoice invoice) {
		...
		sendEmailWithAttachement(invoice.getCustomer().getEmail(), "invoice Email ", " Thanks for the business", true,
				true, invoice.getInvoicePdf());
	}

	@Async
	public void sendInvoiceMailFRomEmail(Email email) {
		log.debug("Sending invoice email to '{}'", email.getUserto());
		sendEmailWithAttachement(email.getUserto(), email.getSubject(), email.getMessage(), true, true,
				email.getAttachment());
	}
